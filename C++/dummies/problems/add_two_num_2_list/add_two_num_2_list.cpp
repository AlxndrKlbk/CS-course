...
int main()
{
    ...
    n1->next = std::shared_ptr< ListNode >( new ListNode( 4 ) ).get();
    n1->next->next = std::shared_ptr< ListNode >( new ListNode( 3 ) ).get();
    n2 = std::shared_ptr< ListNode >( new ListNode( 5 ) );
    n2->next = std::shared_ptr< ListNode >( new ListNode( 6 ) ).get();
    n2->next->next = std::shared_ptr< ListNode >( new ListNode( 4 ) ).get();
    auto* result = solver.addTwoNumbers( n1.get(), n2.get() );
    while( result )
    {
        std::cout<< result->val << " ";
        result = result->next;
